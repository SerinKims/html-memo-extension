...
위치 메모 작성·수정·삭제와 웹페이지/로컬 HTML 오버레이 기반까지 구현되었습니다. Popup은 로컬 파일 접근 권한을 확인하고, 자체 검토 HTML에는 중복 오버레이를 주입하지 않습니다. 내보내기 스키마 v2와 확장 없이 위치·영역·일반 댓글을 추가한 뒤 revision HTML을 다시 다운로드하는 독립 검토 화면 생성기도 포함합니다. 전체 페이지 캡처와 Popup 내보내기 UI 연결은 이후 단계에서 구현합니다.
...
- 웹페이지 위에 메모 작성
- Chrome에서 연 임의의 로컬 HTML 위에 메모 작성
...
- 내보낸 HTML에서 위치·영역·일반 댓글을 추가하고 revision 수정본 다운로드
